...
float black_scholes_monte_carlo(ui64 S0, ui64 K, float T, float r, float sigma, float q, ui64 num_simulations) {
    ...
    ui64 local_num_simulations = num_simulations / size;
    if (rank == size - 1) {
        // Le dernier processus effectue les simulations restantes
        local_num_simulations += num_simulations % size;
    }

    float local_sum_payoffs = 0.0f;

    for (ui64 i = 0; i < local_num_simulations; ++i) {
        float Z = gaussian_box_muller();
        float ST = S0 * exp((r - q - 0.5f * sigma * sigma) * T + sigma * sqrt(T) * Z);
        float payoff = std::max(ST - K, 0.0f);
        local_sum_payoffs += payoff;
    }

    // Réduction : somme de tous les payoffs locaux dans le processus maître (rank 0)
    float global_sum_payoffs = 0.0f;
    MPI_Reduce(&local_sum_payoffs, &global_sum_payoffs, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);

    // Le processus maître calcule le résultat final
    if (rank == 0) {
        return exp(-r * T) * (global_sum_payoffs / num_simulations);
    }

    // Les autres processus retournent 0 (non pertinent)
    return 0.0f;
}
